                ...
                for i in range(mp.nComps):
                    ...
                    if c.val:
                        ...
                        if mp.symbols:
                            ...
                            theseSymbols = '?' * mp.dim
                        func._writeCharFreqToOpenFile(c.val, mp.dim, theseSymbols, sys.stdout, offset=15)
                        print " %s]" % c.spec
                    else:
            ...
            if mp.nRMatrices == 0:
                ...
                for i in range(mp.nRMatrices):
                    ...
                    if theSpec in var.rMatrixProteinSpecs:
                        ...
                        print " " * 15,
                        if mp.dim > 2:
                            func._writeRMatrixTupleToOpenFile(c.val, mp.dim, sys.stdout, offset=15)
                        elif mp.dim == 2:
                            print "dim = 2.  No free values"
                        else:
                            raise P4Error("dim=%s  How did *that* happen?" % mp.dim)
                ...
                for mNum in range(mp.nGdasrvs):
                    ...
                    if mt.free:
                        ...
                        if (mt.val - prams[pos]) > 1.e-10:
                            raise P4Error("restoreFreePrams. bad gdasrv non-restore.")
                        #mt.val = prams[pos]
                        pos += 1
